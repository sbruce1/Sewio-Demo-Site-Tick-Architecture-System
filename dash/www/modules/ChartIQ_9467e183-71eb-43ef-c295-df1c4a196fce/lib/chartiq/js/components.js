...
class ChartIQChart extends HTMLElement {
	 ...
	 */
	startChart(engine, feed, behavior, addOns) {
	...
	configureAddOns(addOns) {
		let stx = this.stx;
		// Use Object.entries in case the object is not enumable like props in React
		Object.entries(addOns).forEach(function(obj) {
			var config = obj[1];
			if (!config) return;
			// Give each config a reference to the engine.
			// If the config is a boolean then it should ONLY be a reference to the engine
			config === true ? config = {stx: stx} : config.stx = stx;
